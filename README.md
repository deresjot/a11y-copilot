...
`a11y-copilot` ist kein automatisches Prüfprogramm. Es ist eine öffentlich einsehbare Sammlung aus Anweisungen, Fachwissen und Quellen für Fragen zur digitalen Barrierefreiheit. Du gibst sie einer KI zusammen mit einer konkreten Aufgabe und dem Material, das sie untersuchen soll.

Die KI kann damit Texte, Gestaltung, Code und Nutzungsschritte analysieren, Anforderungen und Lösungen vorschlagen und – mit dem nötigen Zugriff – Änderungen umsetzen und verfügbare Tests ausführen. Das Ergebnis muss anschließend im echten Produkt geprüft werden. Der Copilot garantiert weder Barrierefreiheit noch Konformität, ersetzt keinen vollständigen Audit oder Tests mit Menschen und leistet keine Rechtsberatung.
...
[ACCESSIBILITY.md](ACCESSIBILITY.md) ist die fachliche Hauptquelle und enthält unter „Arbeitsweise und Antwort“ die gemeinsamen Regeln für Belege, konkrete Kurzantworten und den Abschluss einer Aufgabe. [PROMPT.md](PROMPT.md) ist die direkt verwendbare Startanweisung für einen KI-Chat; [SKILL.md](SKILL.md) steuert die auftragsbezogene Anwendung im Agent-Werkzeug. [references/](references/) enthält Vertiefungen und führt zu WCAG, WCAG-EM, HTML, WAI-ARIA, APG und weiteren Primär- beziehungsweise Methodenquellen. [patterns/](patterns/) behandelt konkrete Komponenten, wenn eine allgemeine Regel nicht ausreicht. [`rulesets/`](rulesets/) trennt fachliche Standardprofile von versionierten Werkzeugadaptern. [`evals/`](evals/) enthält Gegenproben für die wichtigsten Aussagegrenzen der KI. Die öffentliche Einstiegsseite liegt in [index.html](index.html). Eine technische und redaktionelle Zusammenfassung für die Weitergabe an andere Systeme steht in [HANDOFF.md](HANDOFF.md).
...
Der universelle Kern basiert auf [WCAG 2.2](https://www.w3.org/TR/WCAG22/), dem [HTML Standard](https://html.spec.whatwg.org/), [WAI-ARIA](https://www.w3.org/TR/wai-aria-1.2/), [ARIA in HTML](https://www.w3.org/TR/html-aria/) und den Ressourcen des W3C/WAI. Die [WCAG-2.2-Quickref](https://www.w3.org/WAI/WCAG22/quickref/) dient als filterbarer Arbeitszugang; normativ maßgeblich bleibt der dort verlinkte WCAG-Text. [WCAG-EM 2.0](https://www.w3.org/TR/wcag-em-2/) liefert den informativen methodischen Rahmen für umfassende Evaluationen. Der [ARIA Authoring Practices Guide](https://www.w3.org/WAI/ARIA/apg/) und [Inclusive Components](https://inclusive-components.design/) dienen als informative Pattern- und Designreferenzen. Für automatisierte Regressionstests werden die offiziellen Dokumentationen von [Playwright](https://playwright.dev/docs/accessibility-testing) und [axe-core](https://github.com/dequelabs/axe-core) herangezogen.
...
Der Copilot kann Ausschlussrisiken analysieren und daraus Entscheidungen, Anforderungen, Gestaltungshinweise, Inhaltsregeln, Implementierungen, Testpläne, Befunde und Maßnahmen für den Betrieb ableiten. Belastbare Aussagen müssen benennen, was tatsächlich untersucht und getestet wurde. Ein automatisierter Lauf deckt nur einen Teil möglicher Barrieren ab; eine einzelne Kombination aus Browser und assistiver Technologie steht nicht für alle Plattformen; eine Stichprobe belegt keine ungeprüften Bereiche.
